...
1. 현재 Run에 행동이 있었다면 Player의 위치·방향·유지 상호작용을 Echo로 저장합니다.
2. 디자이너가 `persistentFields`로 지정한 오브젝트 속성만 메모리에 저장합니다.
3. 플레이어와 비기억 오브젝트를 레벨 초기 상태로 복원합니다.
4. 기억 오브젝트에는 저장한 상태를 덮어씁니다.
5. 기존 Echo는 현재 위치와 방향을 그대로 유지합니다.
6. 현재 Run의 행동 여부와 경과 시간을 초기화합니다.
7. 리셋 사용 횟수를 1 증가시킵니다.
